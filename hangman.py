...
from models import Game, GameForm,\
    NewGameForm, MakeMoveForm, User,\
    Score, ScoreForm, ScoreForms, UserGameForm, \
    UserGameForms, HighScoreForm,\
    HighScoreForms, GameHistoryForm,\
...
class HangmanApi(remote.Service):
    #API of the hangman game we are going to implement
    ...
    def make_move(self, request):
        ...
        if correctString != correctTarget:
            ...
            if request.guess in game.target:
                ...
                if (str("".join(game.correct))) == (str(game.target.upper())):
                    game.all_guesses.append(
                        'Correctly guessed {} and found {} to win the game!'.format(request.guess.upper(),
                                                                                    game.target.upper()))
                    ...
                    game.end_game(False)
                    return game.to_form(
                        "Congratulations! You guess the word {}. You're a winner!".format(str(game.target.upper())))
                ...
                return game.to_form(
                    "You found a letter in the word! So far you've found {}!".format(str("".join(game.correct))))
            ...
            if game.attempts_remaining < 1:
                ...
                score.put()
                return game.to_form(
                    'Game Over you did not guess the word {} in the allowed {} attemps! Try again, start a new game.'.format(
                        str(game.target.upper()), game.attempts_allowed))
                ...
                game.put()
                return game.to_form(
                    'Try Again, {} not in the target word! You have incorrectly guessed {} so far.'.format(
                        str(request.guess.upper()), str("".join(game.incorrect))))
    ...
    def get_high_scores(self, request):
        ...
        return HighScoreForms(items=[score.to_highform() for score in
                                     Score.query().order(-Score.score).fetch(number_of_results, offset=0)])
